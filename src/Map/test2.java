...
public class test2 {
	...
	private static double Calculatetime(Packman p,Fruit f) {
		...
		double dist=c.distance3D(f.getOrient())-c.get_radius();
		if(dist<=0)return 0;
	...
	private static double greedy(Game g) {
		...
		while(!g.getArray().isEmpty()) {
			...
			g.getArr().get(packmanindex).setOrinet(g.getArray().get(fruitindex).getOrient());
			g.getArr().get(packmanindex).getPath().getArr().add(g.getArray().get(fruitindex).getOrient());
			g.getArr().get(packmanindex).getTime().setSecond((int)min);
			g.getArray().get(fruitindex).getTime().setSecond((int)min);
			g.getArr().get(packmanindex).getPath().getArr().get(g.getArr().get(packmanindex).getPath().getArr().size()-1).getTime().setSecond((int)tmp);
